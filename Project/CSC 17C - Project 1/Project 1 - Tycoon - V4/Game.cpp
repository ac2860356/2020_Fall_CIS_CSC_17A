...
void Game::start() {
    ...
    do {
        ...
        cout << endl << "Everyone's dealt hands" << endl;
        for(std::list<Player *>::iterator o = players->begin(); o != players->end(); o++) {
            cout << (*o)->getTitle() << ". " << (*o)->getName() << ": ";
            (*o)->dspHand();
        }
        cout << endl;
        ...
        if(r > 1) { 
            ...
            cout << endl << "Everyone's new hands" << endl;
            for(std::list<Player *>::iterator o = players->begin(); o != players->end(); o++) {
                cout << (*o)->getTitle() << ". " << (*o)->getName() << ": ";
                (*o)->dspHand();
            }
...
void Game::trick(std::list<Player *>::iterator &i) {
...
void Game::playCards(Player *p) {
    ...
    for(std::list<std::pair<std::string, int>>::iterator i = holding->begin(); i != holding->end(); i++) {
        ...
        //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        //  Practical output, label as most recently played cards (delete comment later)
        //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        cout << (*i).first << endl;
    }
