...
void Game::prntRanks() {
    std::list<std::pair<std::string, int>> *sList;
    sList = new std::list<std::pair<std::string, int>>;
    for(std::list<Player *>::iterator i = players->begin(); i != players->end(); i++) {
        sList->emplace_front((*i)->getName(), (*i)->getTitle());
    }
    srt(sList, sList->size());
    for(std::list<std::pair<std::string, int>>::iterator i = sList->begin(); i != sList->end(); i++) {
        cout << tKey->find((*i).second)->second << ": " << (*i).first << endl;
    }
    cout << endl;
    delete sList;
}
...
void Game::srt(std::list<std::pair<std::string, int>> *&h, int n) {
    if(n == 1) return;
    
    std::list<std::pair<std::string, int>>::iterator i = h->begin(), j = h->begin();
    j++;
